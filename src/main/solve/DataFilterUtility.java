...
public class DataFilterUtility {
    ...
    public static void main(String[] args) {
        try {
            ...
            IClassify classifier = new TypeClassifire();
            Statistics statistics = new Statistics(options.getStatisticsMode());

            try (Saver saver = new Saver(options.getOutputDirectory(), options.getPrefix(),
                    options.isAppend(), StandardCharsets.UTF_8)) {

                DataParser pars = new DataParser(classifier, saver, statistics, StandardCharsets.UTF_8);
            ...
            System.err.println("Ошибка зарытия ресурса\n" + e.getMessage());
        }
    }
}
