...
public class Options {
    ...
    private void validateOutputDirectory(String outputDirectory) {
        ...
        if (!dir.exists()) {
            System.err.println("Ошибка: указанный путь для выходных файлов не существует: " + outputDirectory);
        }

        if (!dir.isDirectory()) {
            System.err.println("Ошибка: указанный путь не является директорией: " + outputDirectory);
        }
    }
